...
@api_view(['POST'])
def green_candle_conf(request):
    last_obj = Vwap_Telegram_data.objects.last()
    idd= last_obj.id
    try:
        if(request.data['title']=="BUY"):
            last_obj.TV_candle_conf_green = True
            last_obj.save()
            print("Updated Green conf success create")
            return HttpResponse(f"Updated Green conf {idd}")
        else:
            return HttpResponse(f"Wrong {idd}")
...
def red_candle_conf(request):
    ...
    try:
        if(request.data['title']=="BUY"):
            last_obj.TV_candle_conf_red = True
            last_obj.save()
            print("Updated Red conf success create")
            return HttpResponse(f"Updated Green conf {idd}")
        else:
            return HttpResponse(f"Wrong {idd}")
        ...
        return HttpResponse("Something went wrong")
@api_view(['POST'])
def consicutive_green_candle(request):
    last_obj = Vwap_Telegram_data.objects.last()
    idd= last_obj.id

    try:
        if(request.data['title']=="BUY"):
            last_obj.TV_candle_exit_2_green = True
            last_obj.save()
            print("Updated Red conf success create")
            return HttpResponse(f"Updated Green conf {idd}")
        else:
        ...
        return HttpResponse("Something went wrong")
@api_view(['POST'])
def consicutive_red_candle(request):
    last_obj = Vwap_Telegram_data.objects.last()
    idd= last_obj.id

    try:
        if(request.data['title']=="BUY"):
            last_obj.TV_candle_exit_2_red = True
            last_obj.save()
            print("Updated Red conf success create")
            return HttpResponse(f"Updated Green conf {idd}")
        else:
...
@api_view(['POST'])
def TV_exit_70_25_rsi(request):
    last_obj = Vwap_Telegram_data.objects.last()
    idd= last_obj.id

    try:
        if(request.data['title']=="BUY"):
            last_obj.TV_exit_70_25_rsi = True
            last_obj.save()
            print("Updated Red conf success create")
            return HttpResponse(f"Updated Green conf {idd}")
        else:
        ...
        return HttpResponse("Something went wrong")
@api_view(['POST'])
def TV_exit_rsi_cross_down(request):
    last_obj = Vwap_Telegram_data.objects.last()
    idd= last_obj.id

    try:
        if(request.data['title']=="BUY"):
            last_obj.TV_exit_rsi_cross_down = True
            last_obj.save()
            print("Updated Red conf success create")
            return HttpResponse(f"Updated Green conf {idd}")
        else:
